#### From 2019/11/09, use this file to clean raw budget data ####
####  Use this file for both national and provincial budget  ####
...
plan <- do.call(rbind, lapply(1:nrow(guide.plan), function(i) {
  ...
  plan <- read.xlsx2(file.plan, sheetIndex=x$sheetIndex, startRow=x$startRow,
                     colClasses=c("numeric", "character", rep("numeric", 15)), encoding="UTF-8")
  plan <- plan[find.numeric(plan[,1]), eval(parse(text=paste("c(",x$keepCol,")", sep="")))]
...
final <- do.call(rbind, lapply(1:nrow(guide.final), function(i) {
  ...
  final <- final[find.numeric(final[,1]), eval(parse(text=paste("c(",x$keepCol,")", sep="")))]
...
prov <- do.call(rbind, lapply(1:nrow(guide.prov), function(i) {
  ...
  prov <- prov[find.numeric(prov[,1]), eval(parse(text=paste("c(",x$keepCol,")", sep="")))]
...
plan <- plan %>%
  ...
  mutate(project.trans.log = neglog(project.trans)) %>%
  ...
  mutate(project.trans.log.lag = lag(project.trans.log, order_by=year)) %>%
  ...
  mutate(project.trans.log.change = project.trans.log - project.trans.log.lag) %>%
  ...
  mutate(project.trans.log.lag2 = lag(project.trans.log, n=2, order_by=year)) %>%
  ...
  mutate(project.trans.log.change2 = project.trans.log - project.trans.log.lag2) %>%
...
prov <- prov %>%
  merge(plan %>% select(total.rev, total.rev.change, total.rev.lag, total.rev.log, total.rev.log.lag, prov, year), by = c("prov", "year"))

#### Save final output
save(plan, file = "National_Budget_Plan_cleaned.RData")
save(final, file = "National_Budget_Final_cleaned.RData")
save(prov, file = "../Vietnam Provincial Budget/Plan/Provincial_Budget_Plan_cleaned.RData")
