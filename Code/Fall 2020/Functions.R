...
negexp <- function(x) sign(x) * (exp(abs(x+.5)))
...
lastterm <- function(x) {
    ...
    last <- ifelse(stringr::str_trim(terms[length(terms)])=="", stringr::str_trim(terms[length(terms)-1]), stringr::str_trim(terms[length(terms)]))
    return(last)
}
