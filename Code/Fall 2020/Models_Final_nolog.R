#### THIS FILE CONTAINS ALL THE MODEL THAT WILL APPEAR IN THE FINAL PAPER ####
library(gsynth)
library(panelView)
...
## but it also screws up the synthetic control results for the main analysis
...
lm_2016_1c <- lm(net.trans ~ defeat + defeat.2016 +
...
# without covariates
lm_2016_pa <- lm(net.trans ~ defeat + defeat.2016 +
                   factor(prov) + factor(year) + factor(prov)*year,
                 data = dat_2016_p) # just lm
summary(lm_2016_pa)

# adding time-variant covariate: lagged total revenue
lm_2016_pb <- lm(net.trans ~ defeat + defeat.2016 +
                   total.rev.lag +
                   factor(prov) + factor(year) + factor(prov)*year,
                 data = dat_2016_p) # just lm
summary(lm_2016_pb)
...
# adding time-invariant covariates instead of prov FE
lm_2016_pc <- lm(net.trans~ defeat + defeat.2016 +
                   num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                   factor(year) + factor(prov)*year,
                 data = dat_2016_p) # just lm
summary(lm_2016_pc)
...
dat_2016_1_placebo2014 <- dat_lme %>%
  mutate(defeat_placebo2014 = defeat.2016 * as.numeric(year == 2014)) %>%
  filter(year < 2015 & year > 2012) %>%
  filter(defeat.2016!=0 | closewin.2016!=0) %>%
  filter(prov!="Ha Noi" & prov!="TP HCM") %>%
  drop_na(net.trans.log)

# without covariates -- time trend does not work due to insufficient dof
lm_2016_1_placebo2014a <- lm(net.trans.change ~ defeat_placebo2014 + defeat.2016 +
                               factor(prov) + factor(year),
                             data = dat_2016_1_placebo2014) # just lm
summary(lm_2016_1_placebo2014a)

# adding time-variant covariate: lagged total revenue
lm_2016_1_placebo2014b <- lm(net.trans.change ~ defeat_placebo2014 + defeat.2016 +
                               total.rev.lag +
                               factor(prov) + factor(year),
                             data = dat_2016_1_placebo2014) # just lm
...
lm_2016_1_placebo2014c <- lm(net.trans.change ~ defeat_placebo2014 + defeat.2016 +
                               num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                               factor(year),
                             data = dat_2016_1_placebo2014) # just lm
...
dat_2016_1_placebo2015 <- dat_lme %>%
  mutate(defeat_placebo2015 = defeat.2016 * as.numeric(year == 2015)) %>%
...
# without covariates
lm_2016_1_placebo2015a <- lm(net.trans ~ defeat_placebo2015 + defeat.2016 +
                               factor(prov) + factor(year) + factor(prov)*year,
...
# adding time-variant covariate: lagged total revenue
lm_2016_1_placebo2015b <- lm(net.trans ~ defeat_placebo2015 + defeat.2016 +
                               total.rev.lag +
                               factor(prov) + factor(year) + factor(prov)*year,
                             data = dat_2016_1_placebo2015) # just lm
...
lm_2016_1_placebo2015c <- lm(net.trans ~ defeat_placebo2015 + defeat.2016 +
                               num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                               factor(year) + factor(prov)*year,
                             data = dat_2016_1_placebo2015) # just lm
...
dat_2016_1_placebo2016 <- dat_lme %>%
  mutate(defeat_placebo2016 = defeat.2016 * as.numeric(year == 2016)) %>%
  filter(year < 2017 & year > 2012) %>%
  filter(defeat.2016!=0 | closewin.2016!=0) %>%
  filter(prov!="Ha Noi" & prov!="TP HCM") %>%
  drop_na(net.trans.log)
# without covariates
lm_2016_1_placebo2016a <- lm(net.trans ~ defeat_placebo2016 + defeat.2016 +
                               factor(prov) + factor(year) + factor(prov)*year,
                             data = dat_2016_1_placebo2016) # just lm
summary(lm_2016_1_placebo2016a)

# adding time-variant covariate: lagged total revenue
lm_2016_1_placebo2016b <- lm(net.trans~ defeat_placebo2016 + defeat.2016 +
                               total.rev.lag +
                               factor(prov) + factor(year) + factor(prov)*year,
                             data = dat_2016_1_placebo2016) # just lm
...
# adding time-invariant covariates instead of prov FEs
lm_2016_1_placebo2016c <- lm(net.trans ~ defeat_placebo2016 + defeat.2016 +
                               num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                               factor(year) + factor(prov)*year,
                             data = dat_2016_1_placebo2016) # just lm
...
for (i in 0:ncol(treatment.2016.randomized)) {
  ...
  purge.y <- lm(net.trans ~ defeat.2016 + 
                  factor(prov) + factor(year) + factor(prov)*year,
                data = dat) # just lm
...
for (i in 0:ncol(treatment.2016.randomized)) {
  ...
  # create residual by purging covariate-based noise
  purge.y <- lm(net.trans.change ~ defeat.2016 +
                  factor(prov) + factor(year),
                data = dat) # just lm
  dat$y.tilde <- resid(purge.y)
...
dat_synth <- plan %>%
  ...
  filter(prov!="Ha Noi" & prov!="TP HCM")
panelView(net.trans ~ defeat, data = dat_synth, index = c("prov", "year"))

## One year change -- no longer a significant result!!!
...
dat_2016synth_1 <- dat_synth %>%
  ...
  #filter(year > 2004) %>%
  drop_na(net.trans)
panelView(net.trans ~ defeat, data = dat_2016synth_1, index = c("prov", "year"))

system.time(
  synth_2016_1 <- gsynth(net.trans ~ defeat + total.rev.lag, 
...
## Persistent change -- no longer a significant result!!!
...
dat_2016synth_p <- dat_synth %>%
  ...
  filter(year > 2004) %>%
  drop_na(net.trans)
panelView(net.trans ~ defeat, data = dat_2016synth_p, index = c("prov", "year"))
panelView(net.trans ~ defeat, data = dat_2016synth_p, index = c("prov", "year"), type = "raw")

system.time(
  synth_2016_p <- gsynth(net.trans ~ defeat + total.rev.lag, 
...
# One year change -- significantly negative even though it should not be
...
system.time(
  synth_2016_1_placebo2014 <- gsynth(net.trans ~ defeat + total.rev.lag, 
...
system.time(
  synth_2016_p_placebo2014 <- gsynth(net.trans ~ defeat + total.rev.lag, 
...
system.time(
  synth_2016_1_placebo2015 <- gsynth(net.trans ~ defeat + total.rev.lag, 
...
system.time(
  synth_2016_p_placebo2015 <- gsynth(net.trans ~ defeat + total.rev.lag, 
...
system.time(
  synth_2016_1_placebo2016 <- gsynth(net.trans ~ defeat + total.rev.lag, 
