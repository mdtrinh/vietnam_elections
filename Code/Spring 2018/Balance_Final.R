...
source("../../Code/Spring 2018/Clean_Covariates.R")
...
list_vars <- c("total.rev", "total.exp",
               ...
               "grdp", "monthly_income_2015","employment_2015",
               "share_ag_land_2015",
...
dat_balance_lme <- plan %>% 
  ...
  select(prov, total.rev, total.exp,
         ...
         defeat.2016) %>%
  inner_join(covs, by = "prov") %>%
  mutate(total.rev = total.rev/1000,
         total.exp = total.exp/1000)


treat_vector_lme <- dat_balance_lme[["defeat.2016"]]
...
balance_rdd <- sapply(list_vars, function(c) {
  ...
  diff = mean_treated - mean_control
  diff_std = diff/diff/sqrt(var_control)
