...
#source("../../Code/Spring 2018/Functions.R") # already called within each file below
source("../../Code/Spring 2018/Clean_Budget.R")
source("../../Code/Spring 2018/Clean_Profile.R")
source("../../Code/Spring 2018/Clean_Results.R")
source("../../Code/Spring 2018/Clean_Leaders.R")
source("../../Code/Spring 2018/Clean_PAPI.R")
source("../../Code/Spring 2018/Clean_PCI.R")
...
# For 2016, possible to calculate true margins and hence "true" close wins
...
plan <- plan %>%
  ...
  merge(bind_rows(provinces2007, provinces2011, provinces2016), by = c("prov", "year"), all.x = T) %>%
  ...
  group_by(prov) %>%
  mutate_at(vars(num.districts:num.closewin), funs(lag(., n=1, order_by = year))) %>%
