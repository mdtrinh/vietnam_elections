...
# without covariates
lm_dev_2016_1a <- lm(dev.exp.log ~ defeat + defeat.2016 +
...
lm_dev_2016_1b <- lm(dev.exp.log ~ defeat + defeat.2016 +
                      total.rev.log.lag +
                  factor(prov) + factor(year),
                data = dat_dev_2016_1) # just lm
...
lm_dev_2016_1c <- lm(dev.exp.log ~ defeat + defeat.2016 +
...
# without covariates
lm_dev_2016_pa <- lm(dev.exp.log ~ defeat + defeat.2016 +
                      factor(prov) + factor(year),
                    data = dat_dev_2016_p) # just lm
summary(lm_dev_2016_pa)

# adding time-variant covariate: lagged total revenue
lm_dev_2016_pb <- lm(dev.exp.log ~ defeat + defeat.2016 +
                      total.rev.log.lag +
                      factor(prov) + factor(year),
                    data = dat_dev_2016_p) # just lm
...
# adding time-invariant covariates instead of prov FE
lm_dev_2016_pc <- lm(dev.exp.log ~ defeat + defeat.2016 +
                       num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                       factor(year),
                     data = dat_dev_2016_p) # just lm
...
system.time(
  synth_dev_2016_1 <- gsynth(dev.exp.log ~ treat + defeat + total.rev.log.lag, 
...
system.time(
  synth_dev_2016_p <- gsynth(dev.exp.log ~ treat + defeat + total.rev.log.lag, 
...
# without covariates
lm_admin_2016_1a <- lm(admin.exp.log ~ defeat + defeat.2016 +
                      factor(prov) + factor(year),
                    data = dat_admin_2016_1) # just lm
summary(lm_admin_2016_1a)

# adding time-variant covariate: lagged total revenue
lm_admin_2016_1b <- lm(admin.exp.log ~ defeat + defeat.2016 +
                      total.rev.log.lag +
                      factor(prov) + factor(year),
                    data = dat_admin_2016_1) # just lm
...
# adding time-invariant covariates instead of prov FE
lm_admin_2016_1c <- lm(admin.exp.log ~ defeat + defeat.2016 +
                       num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                       factor(year),
                     data = dat_admin_2016_1) # just lm
...
# without covariates
lm_admin_2016_pa <- lm(admin.exp.log ~ defeat + defeat.2016 +
                        factor(prov) + factor(year),
                      data = dat_admin_2016_p) # just lm
summary(lm_admin_2016_pa)

# adding time-variant covariate: lagged total revenue
lm_admin_2016_pb <- lm(admin.exp.log ~ defeat + defeat.2016 +
                        total.rev.log.lag +
                        factor(prov) + factor(year),
                      data = dat_admin_2016_p) # just lm
...
# adding time-invariant covariates instead of prov FE
lm_admin_2016_pc <- lm(admin.exp.log ~ defeat + defeat.2016 +
                         num.districts.2016 + num.districts.5.2016 + num.centralnominees.2016 +
                         factor(year),
                       data = dat_admin_2016_p) # just lm
...
system.time(
  synth_admin_2016_1 <- gsynth(admin.exp.log ~ treat + defeat + total.rev.log.lag, 
...
dat_admin_2016synth_p <- dat_admin_synth %>%
  ...
  #filter(defeat.2016!=0 | closewin.2016!=0) %>%
  drop_na(admin.exp.log)
panelView(admin.exp.log ~ defeat, data = dat_admin_2016synth_p, index = c("prov", "year"))

system.time(
  synth_admin_2016_p <- gsynth(admin.exp.log ~ treat + defeat + total.rev.log.lag, 
